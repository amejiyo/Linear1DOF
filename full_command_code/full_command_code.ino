...
void stepperMotor(){
  switch (stepperState){
    case 0:
      ...
      Serial.println((long int)(((desire_pos[substate]*microsteps)/leadscrewPitch) - current_step));
      stepper.move((long int)(((desire_pos[substate]*microsteps)/leadscrewPitch) - current_step));
      Serial.print(current_step);
      Serial.print(" ");
      Serial.print((long int) desire_pos[substate]*microsteps/leadscrewPitch);
      Serial.print(" ");
      Serial.println((long int) (desire_pos[substate]*microsteps/leadscrewPitch) - current_step);
...
void setup() {
  ...
  limitEnd_button.attach(limitEndPin, INPUT_PULLUP);
  start_button.interval(5);
  stop_button.interval(5);
  reset_button.interval(5);
  limitEnd_button.interval(5);
  ...
  // enable interrupt
  // enableInterrupt(5, interruptFunction, CHANGE);     // pin 18 = 3
  enableInterrupt(limitEndPin, resetFunction, CHANGE); 
...
void loop() {
  ...
  start_button.update();
  stop_button.update(); 
  limitEnd_button.update();

  char input_key = keypad.getKey();
  if (reset_button.fell() || limitEnd_button.fell()) {
    resetFunction();
    state = 6;
  }
